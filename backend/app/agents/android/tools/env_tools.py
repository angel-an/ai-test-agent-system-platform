...
提供 adb 设备检查、环境预检、依赖安装、项目初始化等辅助功能
"""
...
async def check_android_env() -> str:
    ...
    Example:
        >>> result = await check_android_env()
    """
    checks = {}
...
async def init_android_project(
    ...
    2. 初始化 package.json（含 @midscene/android, tsx, typescript, dotenv）
    3. 创建 .env 配置文件（配置 Midscene AI 模型参数）
    4. 创建 tests/android 目录
    ...
    Example:
        >>> result = await init_android_project(
    ...
    try:
        ...
        if not package_json.exists():
            package_content = {
                "name": f"android-tests-{project_identifier or 'default'}",
                "version": "1.0.0",
                "private": True,
                "scripts": {
                    "test": "tsx",
