...
用于保存和查询 Android 测试相关的成果物：
- 测试计划 (android_test_plan)
- 测试用例 (android_test_case)
- 测试脚本 (android_test_script)
- 测试报告 (android_test_report)
"""
...
async def save_android_test_plan(
    ...
    Args:
        app_package: 被测应用包名，如 "com.example.app"
    ...
    # 生成 MinIO 对象名称
    object_name = f"android-tests/{project_identifier}/apps/{app_package}/test-plan.{file_extension}"
    ...
    async with async_session_factory() as session:
        ...
        # 生成文件名和描述
        file_name = f"test-plan-{app_package}.{file_extension}"
        format_desc = "Markdown" if plan_format == "markdown" else "JSON"
        description = f"Android 应用 {app_package} 的测试计划 ({format_desc})"
...
async def save_android_test_cases(
    ...
    # 生成 MinIO 对象名称
    object_name = f"android-tests/{project_identifier}/apps/{app_package}/test-cases.json"
    ...
    async with async_session_factory() as session:
        ...
        description = f"Android 应用 {app_package} 的测试用例（共 {len(test_cases)} 个）"
        ...
        if existing_attachment:
            ...
            attachment = Attachment(
                ...
                file_name=f"test-cases-{app_package}.json",
...
async def save_android_test_script(
    ...
    # 生成 MinIO 对象名称
    object_name = f"android-tests/{project_identifier}/apps/{app_package}/test-scripts/{script_name}.{extension}"
    ...
    async with async_session_factory() as session:
        ...
        description = f"Android 应用 {app_package} 的测试脚本 ({script_name}.{extension})"
...
@tool
async def save_android_test_report(
    app_package: str,
    report_content: str,
    ...
    # 生成 MinIO 对象名称
    object_name = f"android-tests/{project_identifier}/apps/{app_package}/test-report.{file_extension}"
    ...
    async with async_session_factory() as session:
        ...
        file_name = f"test-report-{app_package}.{file_extension}"
        format_desc = "HTML" if report_format == "html" else "Markdown"
        description = f"Android 应用 {app_package} 的测试报告 ({format_desc})"
...
async def get_android_artifacts(
    ...
    async with async_session_factory() as session:
        ...
        if sub_function_id:
            ...
            # 通过 object_name 前缀匹配
            prefix = f"android-tests/{project_identifier}/apps/{app_package}/"
            stmt = select(Attachment).where(
