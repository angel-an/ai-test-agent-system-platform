...
# pragma: no cover  MC80OmFIVnBZMlhscm9ua3VMazZjMmhLWXc9PToxNzdjZmRjOQ==
...
shell_backend = GuardedLocalShellBackend(root_dir=Path(settings.api_workspace_root).resolve(),
                                          mode="warn",
                                          inherit_env=True,
...
# pylint: disable  Mi80OmFIVnBZMlhscm9ua3VMazZjMmhLWXc9PToxNzdjZmRjOQ==
...
1. **需求分析报告（仅输出 .md 报告）**
   ...
   - 行动：加载 **requirement_analyzer skill**，生成 Markdown 报告并调用 `save_requirement_analysis` 保存
   - ⛔ 严禁：调用 `save_test_plan` / `save_test_cases` / `save_test_script` 或任何执行测试相关的工具
   - 完成后：把保存路径反馈给用户，并主动询问"是否需要基于该报告继续生成测试计划与测试用例"，等用户确认后再切换流程

2. **生成测试用例 / 测试计划 / 测试脚本（默认流程）**
   - 触发关键词：`生成测试`、`生成测试用例`、`生成测试计划`、`生成脚本`、`output_type=test_case`，或用户提供了 `endpoint_id`
   - 行动：进入下方"标准工作流程"
...
- **📑 需求分析** → 解析需求文档，输出结构化 Markdown 分析报告
...
| 功能 | 工具 | 说明 |
|------|-----|------|
| 📑 保存需求分析 | `save_requirement_analysis` | 保存需求分析 Markdown 报告到本地工作目录 |
...
| Skill | 说明 | 触发条件 |
|-------|------|----------|
| **requirement_analyzer** | 需求文档分析、Markdown 报告生成 | 用户要求"需求分析报告 / 分析报告 / md 格式" |
...
**记住**：
- **需求分析**：解析文档 → 生成 Markdown 报告 → `save_requirement_analysis` 保存 → 询问是否继续生成测试！
...
async def make_agent() -> AsyncIterator[Pregel]:
    ...
    yield api_agent
# pylint: disable  My80OmFIVnBZMlhscm9ua3VMazZjMmhLWXc9PToxNzdjZmRjOQ==
