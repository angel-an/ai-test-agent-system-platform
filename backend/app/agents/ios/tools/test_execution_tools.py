...
对外工具：
- execute_ios_test: 执行测试脚本（支持重试、报告收集）
- collect_ios_report: 收集 Midscene HTML 报告并上传 MinIO
- parse_ios_test_report: 解析 HTML 报告提取统计信息
- batch_execute_ios_tests: 批量执行多个脚本（错误隔离）
"""
...
RETRYABLE_PATTERNS = [
    ...
    re.compile(r"no\s+ios\s+device\s+connected", re.IGNORECASE),
    re.compile(r"device\s+offline", re.IGNORECASE),
    re.compile(r"simulator\s+not\s+found", re.IGNORECASE),
    re.compile(r"wda\s+not\s+responding", re.IGNORECASE),
]
...
NON_RETRYABLE_PATTERNS = [
    ...
    re.compile(r"bundle\s+id\s+not\s+found", re.IGNORECASE),
]
...
def _is_retryable_error(stdout: str, stderr: str, return_code: int) -> bool:
    ...
    可重试：视觉定位抖动、AI 规划超时、设备瞬断、弹窗干扰、WDA 无响应等
    不可重试：脚本语法错误、模块缺失、App 未安装、Bundle ID 错误等
    """
    combined = f"{stdout}\n{stderr}"
...
async def _execute_with_retry(
    ...
    while attempt <= max_retries:
        attempt += 1
        print(f"[iOS Test Execution] 执行尝试 {attempt}/{max_retries + 1}: {script_filename}")
        ...
        if attempt <= max_retries and _is_retryable_error(
            ...
            print(f"[iOS Test Execution] 检测到可重试错误，准备第 {attempt + 1} 次尝试")
            continue
        else:
            print("[iOS Test Execution] 错误不可重试或已达最大重试次数")
            break
...
# ============================================================================
# 报告上传（从 Android 迁移并适配 iOS）
# ============================================================================

async def _save_ios_test_report_zip(
    project_identifier: str,
    report_path: str,
    execution_result: Dict[str, Any],
    project_root: str,
) -> Optional[str]:
    """保存 iOS 测试报告到 MinIO（ZIP 打包）并创建附件记录"""
    try:
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        zip_filename = f"ios_test_report_{timestamp}.zip"
        zip_path = Path(project_root) / zip_filename

        print(f"[iOS Report] 打包测试报告: {report_path} -> {zip_path}")
        ...
        object_name = f"ios-tests/{project_identifier}/reports/test-report-{timestamp}.zip"
        ...
        print(f"[iOS Report] 报告已上传到 MinIO: {object_name}")
        ...
        async with async_session_factory() as session:
            attachment = Attachment(
                ...
                file_name=f"ios-test-report-{timestamp}.zip",
        ...
        try:
            ...
            print(f"[iOS Report] 清理临时 ZIP 文件失败: {e}")

        return object_name

    except Exception as e:
        print(f"[iOS Report] 保存测试报告失败: {e}")
        traceback.print_exc()
        return None
...
async def execute_ios_test(
    ...
    使用 npx vitest run 或 npx tsx 执行已下载到本地测试目录的 Midscene iOS 测试脚本。
    支持失败重试、报告自动收集和上传。
    ...
    Args:
        ...
        app_bundle_id: 被测应用 Bundle ID，用于保存测试报告

    Returns:
        JSON 格式的测试执行结果，包含执行详情、解析结果、报告路径

    Example:
        >>> result = await execute_ios_test(
    ...
    try:
        ...
        env = os.environ.copy()
        if device_udid:
            env["MIDSCENE_IOS_DEVICE"] = device_udid
        ...
        if project_identifier and execution_result.get("report_path"):
            try:
                report_object_name = await _save_ios_test_report_zip(
                ...
                print(f"[iOS Test Execution] 报告上传失败: {e}")
                traceback.print_exc()
...
@tool
async def collect_ios_report(
    report_dir: Optional[str] = None,
    project_identifier: str = "",
    app_bundle_id: str = "",
) -> str:
    """
    收集 Midscene iOS 测试生成的报告
    ...
    Example:
        >>> result = await collect_ios_report(
        ...     project_identifier="proj_001",
        ...     app_bundle_id="com.example.app"
        ... )
    """
    try:
        ...
        for file_path in report_path.rglob("*"):
            if file_path.is_file():
                ...
                # 生成 MinIO 对象名称
                object_name = f"ios-tests/{project_identifier}/apps/{app_bundle_id}/reports/{relative_path.as_posix()}"
...
@tool
async def parse_ios_test_report(
    ...
    Example:
        >>> result = await parse_ios_test_report(
        ...     "ios-tests/proj_001/apps/com.example.app/reports/report-123.html"
        ... )
    """
    try:
        # 从 MinIO 下载报告
...
@tool
async def batch_execute_ios_tests(
    ...
    """
    批量执行多个 iOS 测试脚本（增强版，支持重试和错误隔离）

    Args:
        script_paths: 本地测试脚本路径列表
        device_udid: 可选，指定设备 UDID
        timeout: 每个测试的超时时间（秒）
    ...
    Example:
        >>> result = await batch_execute_ios_tests(
    ...
    try:
        ...
        for script_path in script_paths:
            result_json = await execute_ios_test(
