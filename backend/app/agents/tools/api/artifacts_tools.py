...
def _resolve_workspace_path(file_path: str) -> Path:
    ...
    if os.name == 'nt':  # Windows
        ...
        if file_path.startswith('/') or file_path.startswith('\\'):
            ...
            path = Path(file_path)
# noqa  MC80OmFIVnBZMlhscm9ua3VMazZWbXd5Unc9PTozOTFkNmI3Mg==
...
async def save_test_cases(
    ...
    async with async_session_factory() as session:
        ...
        endpoint_result = await session.execute(endpoint_stmt)
        endpoint = endpoint_result.scalar_one_or_none()
# fmt: off  MS80OmFIVnBZMlhscm9ua3VMazZWbXd5Unc9PTozOTFkNmI3Mg==
        ...
        existing_result = await session.execute(existing_stmt)
        existing_attachment = existing_result.scalar_one_or_none()
# pylint: disable  Mi80OmFIVnBZMlhscm9ua3VMazZWbXd5Unc9PTozOTFkNmI3Mg==
        ...
        # 更新端点的测试用例统计
        # object_name 固定 → JSON 是全量覆盖,计数必须与之同步:赋值而非累加,避免重复生成造成脱离实际的膨胀。
...
async def save_test_script(
    ...
    async with async_session_factory() as session:
        ...
        # 新建 APITest 后先 flush，确保关联 ID 已生成；否则端点会把
        # "None" 写入 api_test_ids，导致后续脚本查询无法命中。
        await session.flush()
        api_test.total_endpoints = 1
        api_test.total_scenarios = endpoint.total_test_cases or 0
...
async def get_endpoint_artifacts(
    ...
    try:
        ...
        return {"error": f"Invalid endpoint_id format: {endpoint_id}. Must be a valid UUID."}
# noqa  My80OmFIVnBZMlhscm9ua3VMazZWbXd5Unc9PTozOTFkNmI3Mg==
