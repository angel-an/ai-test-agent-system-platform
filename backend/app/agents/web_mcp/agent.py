...
# fmt: off  MC80OmFIVnBZMlhscm9ua3VMazZXR3BKVGc9PTowNmU0YmIzNA==
...
from app.agents.mcp_servers import get_playwright_tools
...
from app.agents.tool_guard import wrap_tools_with_guard
from app.agents.web_mcp.tool_error_handler import wrap_tools_with_error_handling
...
shell_backend = GuardedLocalShellBackend(root_dir=Path(settings.web_mcp_workspace_root).resolve(),
                                  mode="enforce",  # rev11：Web agent 固定 enforce
                                  inherit_env=True,
...
# =============================================================================
# MCP 工具单例（进程级缓存，避免每次请求重启 MCP server 进程）
# =============================================================================
...
async def _get_mcp_tools(extra_mcp_servers: dict) -> list:
    """获取 MCP 工具，无 extra_servers 时使用缓存避免重复启动进程"""
    global _mcp_tools_cache
    if not extra_mcp_servers:
        async with _mcp_tools_lock:
            if _mcp_tools_cache is None:
                _mcp_tools_cache = await get_playwright_tools({})
                logger.info(f"MCP tools initialized: {len(_mcp_tools_cache)} tools cached")
            return _mcp_tools_cache
    # 有 extra_servers 时不缓存，每次重新获取
    return await get_playwright_tools(extra_mcp_servers)


# =============================================================================
# 上下文定义
# =============================================================================
# fmt: off  MS80OmFIVnBZMlhscm9ua3VMazZXR3BKVGc9PTowNmU0YmIzNA==
...
3. 执行测试 → `execute_web_script(local_script_path=..., framework="playwright", reporter="html", project_identifier=..., sub_function_id=<子功能ID>)`（rev23：sub_function_id 必填）
...
5. 重新执行测试 → `execute_web_script(local_script_path=..., project_identifier=..., sub_function_id=<子功能ID>)`（rev23：sub_function_id 必填）
...
**⛔ 禁止使用 `planner_setup_page` 和 `generator_setup_page`** — 这两个工具会启动真实测试进程并无限等待，导致 agent 卡死。

**直接使用 `browser_navigate` 开始浏览器操作：**
...
# ❌ 错误：会导致卡死
planner_setup_page(project="chromium")  # 禁止！
generator_setup_page(project="chromium")  # 禁止！
```
...
- 错误："expect is not defined"
  → `browser_evaluate` 已被安全策略禁用（SHELL-POLICY-BANNED），一律改用 `browser_verify_*` 工具
...
**⛔ 禁止脚本内生成报告**：
- 🚫 **禁止在 `test.afterAll()` 中生成 HTML 报告** — 系统会自动生成专业报告
- 🚫 **禁止在脚本中使用 `fs.writeFileSync` 写入 `.html` 文件** — 这会导致报告重复且质量不一致
- 🚫 **禁止在脚本中拼接 HTML 字符串（`<html>`, `<!DOCTYPE html>` 等）** — 不属于脚本职责
- 测试报告由 `execute_web_script` 工具执行后**自动生成**，包含 SVG 图表、统计卡片、截图嵌入
- 脚本只需输出标准 Playwright 测试结果（`console.log` 即可）

**✅ 截图规范**：
- 使用 `page.screenshot({ path: 'screenshots/xxx.png' })` 保存截图
- 截图会被系统自动收集并嵌入到 HTML 报告中
- 截图目录建议使用 `screenshots/` 子目录

**脚本职责边界**：
- ✅ 脚本负责：测试逻辑、断言、截图
- ❌ 脚本不负责：报告生成、HTML 渲染、数据统计可视化
...
| ⛔ 禁止 | `planner_setup_page` | 会卡死，禁止使用 |
| ⛔ 禁止 | `generator_setup_page` | 会卡死，禁止使用 |
...
# fmt: off  Mi80OmFIVnBZMlhscm9ua3VMazZXR3BKVGc9PTowNmU0YmIzNA==

    # 从 configurable 读取外部 MCP Server 配置
    extra_mcp_servers: dict = {}
    if config:
        extra_mcp_servers = config.get("configurable", {}).get("extra_mcp_servers") or {}

    mcp_tools = await _get_mcp_tools(extra_mcp_servers)
    ...
    # 工具安全策略（P0-1）：终局守卫 —— 黑名单工具 final 拒绝 + 导航 origin 白名单
    # 与 error handling 不同：命中策略的调用不进入原始执行，且不提供重试路径
    all_tools = wrap_tools_with_guard(all_tools)
    ...
    yield web_agent
# pylint: disable  My80OmFIVnBZMlhscm9ua3VMazZXR3BKVGc9PTowNmU0YmIzNA==
