...
class IOSApp(Base, UUIDMixin, TimestampMixin):
    """
    iOS App 功能表

    存储 iOS App 业务功能的定义，如"产品管理"、"订单管理"、"用户管理"等
    每个功能可以有多个子功能
    """
    __tablename__ = "ios_apps"
    __table_args__ = {"comment": "iOS App 功能定义表"}
    ...
    bundle_id: Mapped[str | None] = mapped_column(
        String(500),
        nullable=True,
        comment="iOS App Bundle ID，如 com.example.app"
    )

    main_activity: Mapped[str | None] = mapped_column(
        String(500),
        nullable=True,
        comment="主 ViewController 名称或启动页面标识"
    )
    ...
    device_type: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        comment="设备类型，如 iPhone, iPad, Simulator"
    )

    wda_url: Mapped[str | None] = mapped_column(
        String(2048),
        nullable=True,
        comment="WebDriverAgent 服务 URL，如 http://localhost:8100"
    )
    ...
    screens: Mapped[list | None] = mapped_column(
        ...
        nullable=True,
        default=list,
        comment="包含的页面/屏幕列表 [{view_controller, name, type}]"
    )
    ...
    project: Mapped["Project"] = relationship(
        "Project",
        back_populates="ios_apps"
    )
    folder: Mapped["Folder | None"] = relationship(
        "Folder",
        back_populates="ios_apps"
    )
    sub_functions: Mapped[list["IOSSubFunction"]] = relationship(
        "IOSSubFunction",
        back_populates="function",
        cascade="all, delete-orphan",
        order_by="IOSSubFunction.sort_order"
    )

    def __repr__(self) -> str:
        return f"<IOSApp(id={self.id}, identifier={self.identifier}, name={self.name})>"


class IOSSubFunction(Base, UUIDMixin, TimestampMixin):
    """
    iOS App 子功能表
    ...
    """
    __tablename__ = "ios_sub_functions"
    __table_args__ = {"comment": "iOS App 子功能定义表"}
    ...
    function_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("ios_apps.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
        comment="所属功能 ID"
    )
    ...
    target_screens: Mapped[list | None] = mapped_column(
        ...
        nullable=True,
        default=list,
        comment="目标屏幕列表 [{view_controller, name, elements, actions}]"
    )
    ...
    project: Mapped["Project"] = relationship(
        "Project",
        back_populates="ios_sub_functions"
    )
    folder: Mapped["Folder | None"] = relationship(
        "Folder",
        back_populates="ios_sub_functions"
    )
    function: Mapped["IOSApp"] = relationship(
        "IOSApp",
        back_populates="sub_functions"
    )

    def __repr__(self) -> str:
        return f"<IOSSubFunction(id={self.id}, identifier={self.identifier}, name={self.name})>"
