...
# pylint: disable  MC80OmFIVnBZMlhscm9ua3VMazZRVWRuTXc9PToxZDJlNzU0OA==
    ...
    async def get_by_project_with_filters(
        ...
        case_kinds: Optional[list[str]] = None,
        ...
        # 按测试用例 ID 过滤（支持 identifier 和 UUID）
        if test_case_ids:
            # 分离 UUID 和 identifier
            uuid_ids = []
            identifier_ids = []
            for tc_id in test_case_ids:
                try:
                    uuid_ids.append(UUID(tc_id))
                except (ValueError, AttributeError):
                    identifier_ids.append(tc_id)

            conditions = []
            if uuid_ids:
                conditions.append(TestCase.id.in_(uuid_ids))
            if identifier_ids:
                conditions.append(TestCase.identifier.in_(identifier_ids))
        ...
        # 按用例种类过滤
        if case_kinds:
            query = query.where(TestCase.case_kind.in_(case_kinds))
...
# pragma: no cover  MS80OmFIVnBZMlhscm9ua3VMazZRVWRuTXc9PToxZDJlNzU0OA==
        ...
        if search:
            ...
            query = query.where(
                or_(
                    ...
                    TestCase.identifier.ilike(search_pattern),
                    TestCase.description.ilike(search_pattern),
                )
            )

        query = query.offset(offset).limit(limit).order_by(TestCase.created_at.desc())
# pylint: disable  Mi80OmFIVnBZMlhscm9ua3VMazZRVWRuTXc9PToxZDJlNzU0OA==
    ...
    async def add_step(
        ...
        return step
# pylint: disable  My80OmFIVnBZMlhscm9ua3VMazZRVWRuTXc9PToxZDJlNzU0OA==
