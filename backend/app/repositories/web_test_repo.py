...
# pragma: no cover  MC80OmFIVnBZMlhscm9ua3VMazZhRU53VUE9PTozODhjNzg1MQ==
...
class WebTestRepository(BaseRepository[WebTest]):
    ...
    async def get_by_project(
        ...
        """获取项目下的 Web 测试列表（排除 script_path 为空的占位记录）"""
        query = select(WebTest).where(
            WebTest.project_id == project_id,
            WebTest.script_path != "",
            WebTest.script_path.isnot(None),
        )
        ...
        return items, total
# fmt: off  MS80OmFIVnBZMlhscm9ua3VMazZhRU53VUE9PTozODhjNzg1MQ==
...
class WebTestRunRepository(BaseRepository[WebTestRun]):
    ...
    async def get_by_web_test(
        ...
        result = await self.session.execute(query)
        items = list(result.scalars().all())
# noqa  Mi80OmFIVnBZMlhscm9ua3VMazZhRU53VUE9PTozODhjNzg1MQ==
...
class WebTestResultRepository(BaseRepository[WebTestResult]):
    ...
    async def get_by_test_run(
        ...
        return items, total
# pylint: disable  My80OmFIVnBZMlhscm9ua3VMazZhRU53VUE9PTozODhjNzg1MQ==
