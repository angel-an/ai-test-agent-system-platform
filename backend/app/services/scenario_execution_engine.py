...
# pragma: no cover  MC80OmFIVnBZMlhscm9ua3VMazZOMUZCUXc9PTo3YTgyY2M5MQ==
...
# type: ignore  MS80OmFIVnBZMlhscm9ua3VMazZOMUZCUXc9PTo3YTgyY2M5MQ==
    ...
    def _apply_transform(self, value: Any, expression: str) -> Any:
        ...
        使用安全的表达式解析替代 eval，仅支持以下操作：
        - 字符串拼接: 'Bearer ' + value
        - 字符串格式化: f"Bearer {value}"
        - 基本数学运算: value * 1000, value + 1
        - 字符串方法: value.upper(), value.lower(), value.strip()
        - 类型转换: int(value), str(value), float(value)
        """
        try:
            local_vars = {"value": value}
            return self._safe_eval_expression(expression, local_vars)
        except Exception:
            logger.warning(f"转换表达式执行失败: '{expression}'，返回原始值")
            return value

    def _safe_eval_expression(self, expression: str, local_vars: dict) -> Any:
        """安全地评估表达式，替代不安全的 eval"""
        expression = expression.strip()

        # 处理字符串拼接: 'prefix' + value 或 "prefix" + value
        if "+" in expression and "value" in expression:
            parts = expression.split("+")
            result = ""
            for part in parts:
                part = part.strip()
                if part == "value":
                    result += str(local_vars["value"])
                elif (part.startswith("'") and part.endswith("'")) or \
                     (part.startswith('"') and part.endswith('"')):
                    result += part[1:-1]
                else:
                    # 不支持的表达式，尝试安全评估
                    raise ValueError(f"不支持的表达式部分: {part}")
            return result

        # 处理字符串乘法: 'prefix' * value 或 value * 1000
        if "*" in expression and "value" in expression:
            parts = [p.strip() for p in expression.split("*")]
            if len(parts) == 2:
                if parts[0] == "value":
                    multiplier = self._parse_number(parts[1])
                    return local_vars["value"] * multiplier
                elif parts[1] == "value":
                    multiplier = self._parse_number(parts[0])
                    return multiplier * local_vars["value"]

        # 处理字符串方法调用
        method_match = re.match(r'^value\.(\w+)\(\s*\)$', expression)
        if method_match:
            method_name = method_match.group(1)
            val = local_vars["value"]
            if hasattr(val, method_name) and method_name in ('upper', 'lower', 'strip', 'title', 'capitalize'):
                return getattr(val, method_name)()
            raise ValueError(f"不允许的方法调用: {method_name}")

        # 处理类型转换: int(value), str(value), float(value)
        type_match = re.match(r'^(\w+)\(value\)$', expression)
        if type_match:
            type_name = type_match.group(1)
            if type_name in ('int', 'str', 'float', 'bool'):
                return eval(f"{type_name}(local_vars['value'])", {"__builtins__": {}}, {})
            raise ValueError(f"不允许的类型转换: {type_name}")

        # 其他简单表达式，只允许 value 和常量
        if expression == "value":
    ...
    def _substitute_variables(self, obj: Any) -> Any:
        ...
        if isinstance(obj, str):
            ...
            pattern = r'\{\{(\w+(?:\.\w+)*)\}\}'
# type: ignore  Mi80OmFIVnBZMlhscm9ua3VMazZOMUZCUXc9PTo3YTgyY2M5MQ==
...
# fmt: off  My80OmFIVnBZMlhscm9ua3VMazZOMUZCUXc9PTo3YTgyY2M5MQ==
